...
class SAC:
    def __init__(self, state_dim, action_dim, lr=3e-4, gamma=0.99, tau=0.005, alpha=0.2, automatic_entropy_tuning=True):
        ...
        # 让温度系数 α（alpha）自动调整，以维持策略熵接近一个预设的目标值，从而在探索与利用之间自动取得平衡。
        if self.automatic_entropy_tuning:
            # 设置目标熵（Target Entropy），为负的动作维度
            self.target_entropy = -torch.prod(torch.Tensor([action_dim]).to(self.device)).item()
            # 初始化可学习的 log_alpha 参数
    ...
    def update(self, batch_size):
        ...
        # 输出：Q网络损失、策略损失和熵系数损失
        return q1_loss.item(), policy_loss.item(), alpha_loss.item() if alpha_loss is not None else 0
    ...
    for episode in range(episodes):
        ...
        while True:
            ...
            if len(agent.replay_buffer) > batch_size and steps % update_interval == 0:
                q_loss, policy_loss, alpha_loss = agent.update(batch_size) # 更新网络
        ...
        if update_count > 0:
            ...
            avg_q_loss = avg_policy_loss = avg_alpha_loss = 0
        ...
        if (episode + 1) % save_interval == 0:
            agent.save_model(f"SAC_no_value_network/sac_pendulum_episode_{episode+1}.pth")
    
    # 训练完成后保存最终模型
    agent.save_model("SAC_no_value_network/sac_pendulum_final.pth")

    return rewards, q_losses, policy_losses, alpha_losses
...
# 可视化训练结果
def plot_training_results(rewards, q_losses, policy_losses, alpha_losses, window=10):
    """
    绘制训练结果图表
    """
    ...
    plt.tight_layout()
    plt.savefig('SAC_no_value_network/training_results.png', dpi=300, bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    ...
    # 开始训练
    # 输入：环境、代理、训练参数
    # 输出：奖励、Q损失、策略损失、熵系数损失
    rewards, q_losses, policy_losses, alpha_losses = train_sac(
        env, agent, episodes, batch_size, update_interval
    )
    
    # 绘制训练结果
    plot_training_results(rewards, q_losses, policy_losses, alpha_losses)
    
    # 测试训练好的模型
    print("\n开始测试最终模型...")
    test_rewards = test_sac(env, agent, "SAC_no_value_network/sac_pendulum_final.pth", test_episodes=MAX_Test_EPISODES, render=True)
